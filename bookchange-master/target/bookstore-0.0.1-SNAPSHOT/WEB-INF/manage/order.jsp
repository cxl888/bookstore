	...
	<div class="manage">
		...
		<table class="table table-hover">
				<c:forEach items="${bookOrders}" var="bookOrder">
					<tr class="active ">
						<td class="w1 c">借阅号:${bookOrder.oid}</td>
						<td class="w1 c">${bookOrder.date}</td>
						<td class="w1 c">借阅人:${bookOrder.oname}</td>
						<td>常用地址：${bookOrder.adress}</td>
						<c:if test="${bookOrder.status=='已归还'}">
							<td class="w1 c"> 修改 <a href="delserchBookOrder.do?oid=${bookOrder.oid}">删除</a></td>
						</c:if>
						<c:if test="${bookOrder.status!='已归还'}">
