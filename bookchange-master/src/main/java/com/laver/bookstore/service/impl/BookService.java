...
public class BookService implements IBookService{
	...
	public int reduceStore(Integer bid, Integer number) {
		...
		//System.out.println(book.getStore());
		//User user = (User) bookMapper.selectByprovider(book.getProvider());
		//user.setContribution(user.getContribution()+1);
		return bookMapper.updateByPrimaryKey(book);
	}

	@Override
	public int addStore(Integer bid, Integer number) {
		// TODO Auto-generated method stub
		//System.out.println("addstore"+bid + number);
		Book book = bookMapper.selectByPrimaryKey(bid);
		book.setStore(book.getStore()+number);
		//System.out.println(book.getStore());
		return bookMapper.updateByPrimaryKey(book);
	...
	@Override
	public List<Book> findBookProvider(String provider) {
		return bookMapper.selectByprovider(provider);
	}
}
